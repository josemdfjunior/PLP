...
evaluate_aux(Elem) ->
    if
        Elem == "+" ->
            ...
            Pid_soma = spawn(?MODULE, soma, [self(), A, B]),
            receive
              {Pid_soma, Result} ->
        ...
        Elem == "-" ->
            ...
            B = lists:last(ListaA),
            ListaB = lists:droplast(ListaA),
            Pid_sub = spawn(?MODULE, subtracao, [self(), A, B]),
            receive
              {Pid_sub, Result} ->
                  put("stack", ListaB ++ [Result])
            end;
        Elem == "*" ->
            ...
            B = lists:last(ListaA),
            ListaB = lists:droplast(ListaA),
            Pid_multiplicacao = spawn(?MODULE, multiplicacao, [self(), A, B]),
            receive
        ...
        Elem == "/" ->
            ...
            B = lists:last(ListaA),
            ListaB = lists:droplast(ListaA),
            Pid_div = spawn(?MODULE, divisao, [self(), A, B]),
            receive
              {Pid_div, Result} ->
                  put("stack", ListaB ++ [Result])
            end;
